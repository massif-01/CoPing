...
public enum AppText {
    ...
    public static var invalidNtfyTopic: String {
        text(
            "NTFY Topic 必须由 CoPing 自动生成。",
            "The NTFY topic must be generated by CoPing."
        )
    }

    public static var ntfyConfigurationReadFailed: String {
        text(
            "无法读取本地 NTFY 配置，原文件未被修改。请检查配置文件后重试。",
            "Unable to read the local NTFY configuration. The original file was not modified. Check the configuration file and try again."
        )
    }

    public static var invalidNtfyResponse: String {
        text(
            "NTFY 返回了无法识别的响应。",
            "NTFY returned an unrecognized response."
        )
    }

    public static var ntfyRedirectRejected: String {
        text(
            "NTFY 服务返回了重定向，CoPing 已拒绝继续发送。",
            "The NTFY service redirected the request. CoPing refused to continue."
        )
    }

    public static var ntfyRateLimited: String {
        text(
            "NTFY 暂时限制了发送频率，请稍后重试。",
            "NTFY temporarily rate-limited delivery. Try again later."
        )
    }

    public static func ntfyHTTPFailure(_ status: Int) -> String {
        text("NTFY 请求失败（HTTP \(status)）。", "NTFY request failed (HTTP \(status)).")
    }
    ...
    public static var recentPushPartiallyFailed: String {
        text("最近推送部分成功", "Recent push partially succeeded")
    }
    ...
    public static var ntfySettingsDescription: String {
        text(
            "无需账号或自备服务器；在手机 ntfy App 中订阅下方 Topic。",
            "No account or personal server required. Subscribe to the topic below in the ntfy phone app."
        )
    }
    ...
    public static var notificationsDisabledHelp: String {
        text(
            "关闭后仍会接收 Codex 事件，但不会发送到已启用的通知通道。",
    ...
    public static var enableBarkHelp: String {
        text(
            "保存有效的 Bark 配置后即可启用；不会影响 NTFY。",
            "Save a valid Bark configuration before enabling it. NTFY is unaffected."
        )
    }
    public static var enableNtfy: String {
        text("启用 NTFY", "Enable NTFY")
    }
    public static var enableNtfyHelp: String {
        text(
            "保存 Topic 后即可启用；可与 Bark 同时发送。",
            "Save the topic before enabling it. NTFY can send alongside Bark."
        )
    }
    public static var ntfyOfficialService: String {
        text("官方服务", "Official service")
    }
    public static var ntfyOfficialServiceHelp: String {
        text(
            "CoPing 仅连接官方 https://ntfy.sh。",
            "CoPing connects only to the official https://ntfy.sh service."
        )
    }
    public static var ntfyTopic: String {
        "Topic"
    }
    public static var ntfyTopicHelp: String {
        text(
            "由 CoPing 强随机生成且不可手动修改。复制后，在手机 ntfy App 中订阅。",
            "Generated securely by CoPing and not manually editable. Copy it and subscribe in the ntfy phone app."
        )
    }
    public static var copyNtfyTopic: String {
        text("复制 Topic", "Copy topic")
    }
    public static var regenerateNtfyTopic: String {
        text("重新生成 Topic", "Generate a new topic")
    }
    ...
    public static var ntfySettingsSaved: String {
        text("NTFY Topic 已保存。", "NTFY topic saved.")
    }
    public static var ntfyTopicCopied: String {
        text("NTFY Topic 已复制。", "NTFY topic copied.")
    }
    public static var ntfyTopicRegenerated: String {
        text(
            "已生成新的 Topic，并关闭 NTFY。请先在手机订阅，再保存、测试并重新启用。",
            "A new topic was generated and NTFY was disabled. Subscribe on your phone, then save, test, and re-enable it."
        )
    }
    ...
    public static var ntfyNotConfigured: String {
        text("NTFY 未配置", "NTFY is not configured")
    }
    public static var ntfyTopicNotSaved: String {
        text(
            "请先保存当前 Topic，再启用 NTFY。",
            "Save the current topic before enabling NTFY."
        )
    }
    public static var noPushChannelEnabled: String {
        text("没有启用通知通道", "No notification channel is enabled")
    }
    ...
    public static func localizedHistoryDetail(_ stored: String) -> String {
        let exactMappings: [([String], String)] = [
            ...
            (["NTFY 未配置", "NTFY is not configured"], ntfyNotConfigured),
            (["没有启用通知通道", "No notification channel is enabled"], noPushChannelEnabled),
        ...
        if stored.hasPrefix("NTFY"), stored.contains("HTTP"), let number {
            return ntfyHTTPFailure(number)
        }
        return stored
    }

    public static var deliverySent: String {
        text("已发送", "Sent")
    }
    public static var deliveryFailed: String {
        text("失败", "Failed")
    }
    public static var deliverySkipped: String {
        text("已跳过", "Skipped")
    }
    ...
    public static var ntfyTestNotificationBody: String {
        text("NTFY 测试通知", "NTFY test notification")
    }
